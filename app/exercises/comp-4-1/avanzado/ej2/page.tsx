...
export default function Page() {
  ...
  const handleNext = async () => {
    setPoint(COMPETENCE, LEVEL, 2, 0) // Pregunta 2, punto 0

    const sid = sessionId || (user ? localStorage.getItem(sessionKeyFor(user.uid)) : null)
    if (sid) {
      try {
        await markAnswered(sid, 1, false) // índice 1 = P2
      } catch (e) {
        console.warn("No se pudo marcar P2:", e)
      }
    }

    router.push("/exercises/comp-4-1/avanzado/ej3") // siguiente
  }
        ...
        <Card className="bg-white shadow-2xl rounded-2xl border-0 ring-2 ring-[#286575]/20">
          <CardContent className="p-6 space-y-6">
            ...
            <div className="space-y-4">
              {/* Contenido de la P2 por completar */}
            </div>
