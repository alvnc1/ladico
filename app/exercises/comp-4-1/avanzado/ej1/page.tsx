...
export default function Page() {
  ...
  const handleNext = async () => {
    // Por ahora no evalúa nada, solo pasa
    setPoint(COMPETENCE, LEVEL, 1, 0) // Pregunta 1, punto en 0

    const sid = sessionId || (user ? localStorage.getItem(sessionKeyFor(user.uid)) : null)
    if (sid) {
      try {
        await markAnswered(sid, 0, false)
      } catch (e) {
        console.warn("No se pudo marcar P1:", e)
      }
    }

    router.push("/exercises/comp-4-1/avanzado/ej2") // siguiente pregunta
  }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pb-6">
        <Card className="bg-white shadow-2xl rounded-2xl border-0 ring-2 ring-[#286575]/20">
          <CardContent className="p-6 space-y-6">
            {/* Aquí irá el enunciado y preguntas */}
            ...
            <div className="space-y-4">
              {/* Espacio vacío para colocar las preguntas u opciones más adelante */}
            </div>
