...
export default function LadicoDeletedListRecoveryExercise() {
  ...
  // ----- Estados para respuesta/finalización (ahora en el padre) -----
  ...
  // input del usuario y feedback
  const [input, setInput] = useState("");
  const [result, setResult] = useState<"idle" | "ok" | "err">("idle");

  // el hijo (desktop) nos avisa si el archivo fue restaurado al escritorio
  ...
  // ====== Finalizar (en el padre) ======
  const ANSWER = "papel higiénico";
  const normalize = (s: string) =>
    s.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/\s+/g, " ").trim();

  const handleFinalize = async () => {
    const ok = restored && normalize(input) === normalize(ANSWER);
    setResult(ok ? "ok" : "err");
    ...
    const QUESTION_IDX_ONE = 3;  // P3 (1-based)
    ...
    let totalPts = levelPoints(prog);
    let passed = isLevelPassed(prog);
    let score = Math.round((totalPts / 3) * 100);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pb-6 sm:pb-8">
        <Card className="bg-white shadow-2xl rounded-2xl sm:rounded-3xl border-0 transition-all duration-300 ring-2 ring-[#286575] ring-opacity-30 shadow-[#286575]/10">
          <CardContent className="p-4 sm:p-6 lg:p-8">
            ...
            {/* Desktop (el hijo solo maneja la escena) */}
            <DesktopRecoveryScene onRestoredChange={setRestored} />
...
/* =========================================================================
   Hijo: solo la ESCENA del escritorio (sin botón ni input)
   ========================================================================= */
function DesktopRecoveryScene({
  onRestoredChange,
}: {
  onRestoredChange?: (restored: boolean) => void;
}) {
  ...
  // avisar al padre si el archivo fue restaurado
  useEffect(() => {
    onRestoredChange?.(icons.some((x) => x.id === "lista"));
  }, [icons, onRestoredChange]);
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border bg-black/5">
      ...
      >
        ...
        {/* Ventanas */}
        {open === "recycle" && (
          <Window title="Recycle Bin" size="small" onClose={() => setOpen(null)}>
            <div className="text-sm">
              {!icons.some((x) => x.id === "lista") && (
                <div
                  className="inline-flex flex-col items-center p-3 rounded-lg hover:bg-gray-50 select-none cursor-grab active:cursor-grabbing"
                  draggable
                  onDragStart={(e) => {
...
function Window({
  ...
  onClose: () => void;
  /** "normal" (llena el panel) o "small" (ventana compacta centrada) */
  size?: "normal" | "small";
}) {
  const wrapperClass =
    size === "small"
      ? "absolute left-1/2 top-24 -translate-x-1/2 w-[480px] max-w-[95vw] h-[320px] max-h-[70vh]"
      : "absolute inset-x-6 top-20 bottom-12";

  return (
    <div className={`${wrapperClass} bg-white border shadow-2xl rounded-xl flex flex-col overflow-hidden z-20`}>
