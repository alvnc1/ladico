...
function normalizeNum(s: string) {
  // Acepta solo dígitos y signo negativo (por si acaso)
...
const ProgrammingExercise2 = forwardRef<ProgrammingExercise2Handle, {}>(
  function ProgrammingExercise2(_, ref) {
    ...
    const rowStyle =
      "grid grid-cols-1 sm:grid-cols-[minmax(240px,1fr),20rem] items-center gap-2"
    const fieldStyle =
      "px-2 py-2 rounded-md border border-gray-300 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#286575] w-full sm:w-80 text-sm"
    ...
    return (
      <div className="space-y-6">
        {/* Pseudocódigo */}
        <div
          className="bg-[#f3fbfb] border-l-4 border-[#286575] p-4 rounded-r-xl shadow-sm"
          role="region"
          aria-label="Pseudocódigo del ejercicio"
        >
          <div className="text-gray-800 text-sm leading-6 space-y-1 font-mono">
            <p>x ← 2</p>
            <p>y ← 3</p>
            <p>z ← 4</p>
            <br />
        ...
        {/* Preguntas */}
        <div className="space-y-4 text-sm text-gray-700" role="group" aria-labelledby="preguntas-label">
          <span id="preguntas-label" className="sr-only">Preguntas del ejercicio</span>
          ...
          <div className={rowStyle}>
            ...
            <input
              ...
              onChange={(e) =>
                setAnswers((prev) => ({ ...prev, firstLine: e.target.value }))
              }
              className={fieldStyle}
              placeholder='Escriba aquí'
              aria-describedby="help-ans1"
            />
          </div>
          ...
          <div className={rowStyle}>
            ...
            <input
              ...
              onChange={(e) =>
                setAnswers((prev) => ({ ...prev, secondLine: e.target.value }))
              }
              className={fieldStyle}
              placeholder="Escriba aquí"
              aria-describedby="help-ans2"
            />
          </div>
          ...
          <div className={rowStyle}>
            ...
            <input
              ...
              onChange={(e) =>
                setAnswers((prev) => ({ ...prev, thirdLine: e.target.value }))
              }
              className={fieldStyle}
              placeholder="Escriba aquí"
              aria-describedby="help-ans3"
            />
          </div>
        </div>
